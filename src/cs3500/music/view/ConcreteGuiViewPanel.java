...
public class ConcreteGuiViewPanel extends JPanel {
  ...
  private void drawNotes(Graphics g) {
    ...
    for (int i = 0; i < loNote.size(); i++) {
      ...
      currentRectangle = new Rectangle((currentNote.getStartBeat()) * BEAT_WIDTH + FIRSTX,
      ...
      g.fillRect((currentNote.getStartBeat()) * BEAT_WIDTH + FIRSTX, noteYPosition,
  ...
  g.setColor(Color.BLUE);
    int height = NOTE_HEIGHT * model.range().size();
  for (int i = 0; i < model.loRepeat().size(); i++) {
    Repeat currentRepeat = model.loRepeat().get(i);
    g.fillRect(2 + currentRepeat.startRepeat() * BEAT_WIDTH + 2 * BEAT_WIDTH, NOTE_HEIGHT,
            2, height);
    g.fillRect(currentRepeat.endRepeat() * BEAT_WIDTH + 2 * BEAT_WIDTH + 8, NOTE_HEIGHT,
            2, height);
  }
  }

  private void drawEndings(Graphics g) {
    int height = NOTE_HEIGHT * model.range().size();
    for (int i = 0; i < model.loEndings().size(); i++) {
      Repeat currentEnd = model.loEndings().get(i);
      if (i == 0) {
        g.setColor(Color.GREEN);
        g.fillRect(2 + currentEnd.startRepeat() * BEAT_WIDTH + 2 * BEAT_WIDTH, NOTE_HEIGHT,
                2, height);
      } else {
        g.setColor(Color.ORANGE);
        g.fillRect(2 + currentEnd.startRepeat() * BEAT_WIDTH + 2 * BEAT_WIDTH, NOTE_HEIGHT,
                2, height);
        g.fillRect(currentEnd.endRepeat() * BEAT_WIDTH + 2 * BEAT_WIDTH + 8, NOTE_HEIGHT,
                2, height);
      }
    }
  }
